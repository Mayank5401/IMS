...
def view_sales_ui():
    ...
    tk.Label(main_window, text="Sales Records", font=("Arial", 16, "bold"),bg=bg_color, fg=fg_color).pack(pady=10)
...
def main_menu():
    clear_screen()
    apply_theme()
    tk.Label(main_window, text="Inventory Management", font=("Calibri", 20, "bold"), fg= fg_color, bg=bg_color).pack(pady=10)
    ...
    for text, command in buttons:
        tk.Button(main_window, text=text, command=command, font=("Arial", 12), width=20, height=2, bg="#007acc", fg=fg_color).pack(pady=5)
